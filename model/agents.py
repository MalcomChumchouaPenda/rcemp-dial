...
class RegulatorAgent(BasicAgent):  
    ...
    def __init__(self, name, model, problem):
        ...
        self.customers = [CustomerAgent(model, o) for o in problem.orders]
        self.producers = [ProducerAgent(model, m) for m in problem.machines]
        self.mainteners = [MaintenerAgent(model, m) for m in problem.maintenances]
    ...
    def step(self):      
        ...
        while count_tm():
            ...
            if count_loop > MAX_COUNT_LOOP:
                break
                # raise RuntimeWarning(f'too much maintenance {count_tm()}')
        ...
        while penalizable:
            ...
            if count_loop > MAX_COUNT_LOOP:
                break
                # raise RuntimeWarning(f'too much penalities {sum_pn()}')
    ...
    def _calc_forced_time(self):
        ...
        r_max = max([c.order.release_date for c in customers])
        if r_max == 0:
            r_max = 1
        factor = H/r_max
...
class RessourceUser:
    ...
    def _calc_fp(self, wp, PL, pmax=None, forced=None):
...
class CustomerAgent(RessourceUser, BasicAgent):
    ...
    def accept(self, forced=None):
        ...
        index = 0
        pmax = None if len(planned) == 0 else planned[-1]
        PL = env.read_proposals(self.unique_id) 
        
        log_info = self.log_info
        calc_fp = self._calc_fp
        accept_proposal = env.accept_proposal
        
        for tid, wp in unplanned: 
            fp = calc_fp(wp, PL[tid], pmax, forced=forced)
            if fp is None:
...
class ProducerAgent(RessourceUser, RessourceWrapper, BasicAgent):
    ...
    def schedule(self):
        ...
        if len(performable_tfs) == 0 and len(positionned_tfs)==0:
            for wp in env.read_wishes(Cap):
                if wp.rid == 0:
                    performable_tfs[wp.tid] = wp
        
        calc_ep = self._calc_ep
        calc_pp = self._calc_pp
        duration = 0
...
class MaintenerAgent(RessourceWrapper, BasicAgent):
    ...
    def schedule(self):
        ...
        env = self.env
        Cap = self.capabilities
        performable = [wp for wp in env.read_wishes(Cap)]
