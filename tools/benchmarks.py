...
from .databases import SqliteDb, MySqlDb
...
class BenchmarkGenerator:
    ...
    def __init__(self, filter_='%', verbose=None):
        super().__init__()
        self.filter = filter_
...
class MySqlGenerator(BenchmarkGenerator):
    
    def __init__(self, filter_='%', verbose=None):
        super().__init__(filter_=filter_, verbose=verbose)
        name = self.__class__.__qualname__
        self.db = MySqlDb(name, verbose=verbose)
...
class SqliteGenerator(BenchmarkGenerator):
    
    def __init__(self, dbdir, filter_='%', verbose=None):
        super().__init__(filter_=filter_, verbose=verbose)
        self.dbdir = dbdir
    ...
    def clear_problems(self):
        ...
        for filename in os.listdir(filedir):
            if filename.endswith('.db') and re.match(filter_, filename):
                filepath = os.path.join(filedir, filename)
                os.unlink(filepath)
    
    
class ArchCoud2001(MySqlGenerator):
...
class BencheikhAl2022(MySqlGenerator):
...
class Dialysis2021(SqliteGenerator):
    
    def __init__(self, filter_='%', verbose=None):
        super().__init__(DATA_DIR,  filter_=filter_,  verbose=verbose)
        self.datadir = os.path.join(DATA_DIR, 'HGD_dialysis_2018_2021')
    ...
    def generate_problems(self):
        ...
        filter_ += '$'
        x, y = 0, len(threads)
        # for period in patients_data.index[:2]:
    ...
    def _generate_problem(self, name, patients_row, machines_group, 
        ...
        path = os.path.join(self.dbdir, name + '.db')
        db = SqliteDb(path, verbose=self.verbose)
        session = db.connect()
        uid = sch.Problem.next_uid()
        pb = sch.Problem(uid=uid, name=name)
        session.add(pb)
    ...
    def find_problems(self):
        ...
        for name in os.listdir(dbdir):
            if name.startswith('dial_S'):
                pb_ids.append(os.path.join(dbdir, name))
        return pb_ids
